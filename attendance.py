...
class Attendance:
    def __init__(self, root):
        ...
        self.var_atten_Student_no = StringVar()
        ...
        left_inside_frame.place(x=5, y=155, width=665, height=365)
        ...
        # Name
        Student_noLabel = Label(left_inside_frame, text="Student Number:", font=("times new roman", 12, "bold"), bg="white")
        Student_noLabel.grid(row=0, column=2, padx=10, pady=5, sticky=W)
        
        atten_Student_no = ttk.Entry(left_inside_frame, textvariable=self.var_atten_Student_no, width=20, font=("times new roman", 12, "bold"))
        atten_Student_no.grid(row=0, column=3, padx=10, pady=5, sticky=W)
        ...
        atten_date.grid(row=2, column=3)
        
        # Attendance Status
        atten_statusLabel = Label(left_inside_frame, text="Attendance Status:", font=("times new roman", 12, "bold"), bg="white")
        atten_statusLabel.grid(row=3, column=0, padx=10, pady=5, sticky=W)
        ...
        btn_frame.place(x=5, y=245, width=650, height=35)
        ...
        table_frame.place(x=5, y=5, width=630, height=510)
        
        # ===================== Scroll Bar Table =========================
        scroll_x = ttk.Scrollbar(table_frame, orient=HORIZONTAL)
        scroll_y = ttk.Scrollbar(table_frame, orient=VERTICAL)
        
        self.AttendanceReportTable = ttk.Treeview(table_frame, column=("id", "Student_no", "name", "department", "time", "date", "attendance"), xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)
        ...
        self.AttendanceReportTable.heading("Student_no", text="Student Number")
        ...
        self.AttendanceReportTable.column("Student_no", width=100)
    ...
    def get_cursor(self, event=""):
        ...
        self.var_atten_Student_no.set(rows[1])
                ...
                if Update > 0:
                    ...
                    for record in data:
                        if record[0] == self.var_atten_id.get():
                            record[1] = self.var_atten_Student_no.get()
                            record[2] = self.var_atten_name.get()
                            record[3] = self.var_atten_dep.get()
                            record[4] = self.var_atten_time.get()
                            record[5] = self.var_atten_date.get()
                            record[6] = self.var_atten_status.get()
                            break  #Stop searching once the record is found
                    
                    with open('attendance.csv', 'w', newline='') as file:
                        fieldnames = ['Attendance ID', 'Student Number', 'Name', 'Department', 'Time', 'Date', 'Attendance']
